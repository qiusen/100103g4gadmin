...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/buyExplain/buyExplainAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//buyExplain/buyExplainAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/buyExplain/buyExplainAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { $.ligerDialog.warn('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/buyExplain/buyExplainAction!edit.${actionExt}?id=" + row.id;
	}
...
<form name="buyExplainForm" id="buyExplainForm" method="post" action="buyExplainAction.${actionExt}" >
...
<div id="searchbar">
内容：<input id="buyExplain.content" type="text" name="buyExplain.content" value="${buyExplain.content }"/>
