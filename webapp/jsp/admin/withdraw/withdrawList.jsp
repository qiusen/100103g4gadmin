...
function f_initGrid(){
    g = manager = $("#maingrid").ligerGrid({
		...
		{ display: '金额', name: 'haveCoin',type: 'text' , width: '15%'},
		{ display: '转出金额', name: 'coin',type: 'text' , width: '15%'},
		{ display: '剩余金额', name: 'leftCoin',type: 'text' , width: '15%'},
		{ display: '备注', name: 'remark',type: 'text' , width: '15%'},
		{ display: '申请时间', name: 'createtime',type: 'text' , width: '15%'},
...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/withdraw/withdrawAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//withdraw/withdrawAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/withdraw/withdrawAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { $.ligerDialog.warn('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/withdraw/withdrawAction!edit.${actionExt}?id=" + row.id;
	}
...
<form name="withdrawForm" id="withdrawForm" method="post" action="withdrawAction.${actionExt}" >
...
<div id="searchbar">
编号：<input id="withdraw.code" type="text" name="withdraw.code" value="${withdraw.code }"/>
