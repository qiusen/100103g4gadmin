...
function f_initGrid(){
    g = manager = $("#maingrid").ligerGrid({
		...
		{ display: '回复', name: 'revet',type: 'text' , width: '15%'},
		{ display: '创建人', name: 'creator',type: 'text' , width: '15%'},
		{ display: '创建时间', name: 'createtime',type: 'text' , width: '15%'},
		{ display: '回复人', name: 'revetor',type: 'text' , width: '15%'},
		{ display: '回复时间', name: 'revettime',type: 'text' , width: '15%'}
        ],
        ...
        //{ text: '增加', value:'add', click: itemclick, icon: 'add', img: '${base}/ligerUI/skins/icons/add.gif' },
                           { text: '回复', value:'edit', click: itemclick, icon: 'edit', img: '${base}/ligerUI/skins/icons/edit.gif' }//,
...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/comment/commentAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//comment/commentAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/comment/commentAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { $.ligerDialog.warn('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/comment/commentAction!edit.${actionExt}?id=" + row.id;
	}
	if(item.value=='delete'){
		var row = manager.getSelectedRow();
        if (!row) {$.ligerDialog.warn('请选择行'); return; }
		$.ligerDialog.confirm('确认删除模块 ' + row.type + ' 的信息？', function (yes) {
            if(yes==true){
            	window.location="${base}/admin/comment/commentAction!delete.${actionExt}?id=" + row.id;
            }
        });
	}
