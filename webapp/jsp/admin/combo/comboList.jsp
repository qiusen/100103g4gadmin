...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/combo/comboAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//combo/comboAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/combo/comboAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { $.ligerDialog.warn('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/combo/comboAction!edit.${actionExt}?id=" + row.id;
	}
	if(item.value=='delete'){
		...
		$.ligerDialog.confirm('确认删除？', function (yes) {
            if(yes==true){
            	window.location="${base}/admin/combo/comboAction!delete.${actionExt}?id=" + row.id;
            }
        });
	}
...
<form name="comboForm" id="comboForm" method="post" action="comboAction.${actionExt}" >
...
<div id="searchbar">
名称：<input id="combo.name" type="text" name="combo.name" value="${combo.name }"/>
