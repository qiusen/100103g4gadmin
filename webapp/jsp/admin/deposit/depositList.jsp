...
function f_initGrid(){
    g = manager = $("#maingrid").ligerGrid({
		...
		{ display: '报单币', name: 'taxCoin',type: 'text' , width: '15%'},
		{ display: '说明', name: 'des',type: 'text' , width: '15%'},
...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/deposit/depositAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//deposit/depositAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/deposit/depositAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { $.ligerDialog.warn('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/deposit/depositAction!edit.${actionExt}?id=" + row.id;
	}
	if(item.value=='delete'){
		...
		$.ligerDialog.confirm('确认删除模块 ' + row.code + ' 的信息？', function (yes) {
            if(yes==true){
            	window.location="${base}/admin/deposit/depositAction!delete.${actionExt}?id=" + row.id;
            }
        });
	}
...
<form name="depositForm" id="depositForm" method="post" action="depositAction.${actionExt}" >
...
<div id="searchbar">
编号：<input id="deposit.code" type="text" name="deposit.code" value="${deposit.code }"/>
