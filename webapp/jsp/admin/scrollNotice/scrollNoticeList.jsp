...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/scrollNotice/scrollNoticeAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//scrollNotice/scrollNoticeAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/scrollNotice/scrollNoticeAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { $.ligerDialog.warn('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/scrollNotice/scrollNoticeAction!edit.${actionExt}?id=" + row.id;
	}
	if(item.value=='delete'){
		...
		$.ligerDialog.confirm('确认删除模块 ' + row.content + ' 的信息？', function (yes) {
            if(yes==true){
            	window.location="${base}/admin/scrollNotice/scrollNoticeAction!delete.${actionExt}?id=" + row.id;
            }
        });
	}
...
<form name="scrollNoticeForm" id="scrollNoticeForm" method="post" action="scrollNoticeAction.${actionExt}" >
...
<div id="searchbar">
内容：<input id="scrollNotice.content" type="text" name="scrollNotice.content" value="${scrollNotice.content }"/>
